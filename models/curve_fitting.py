...
def curve_fitting(data):
    ...
    data_len = len(data)
    sample_len = max(int(data_len * 0.10), 1)  # ambil minimal 1 data kalau terlalu kecil
    data = data[-sample_len:]  # ambil dari belakang (data terbaru)
    # Proses data
    data = data.reset_index()
    ...
    y = data['Close'].to_numpy().flatten() #mengubah row pandasr menjadi array numpy .
    # flatten dipake buat memastikan array 1 dimensi supaya bisa di proses sama polyfit
    ...
    dates = data['Price']  # Ambil kolom tanggals
    ...
    # Tampilkan label x sebagai tanggal - FIX BAGIAN INI
    step = max(1, len(x) // 10)  # Ambil setiap step data untuk x-ticks
    tick_positions = x[::step]
    tick_labels = dates.iloc[::step].dt.strftime('%Y-%m-%d')

    plt.figure(figsize=(10, 5))
    plt.plot(x, y, 'bo-', label='Data Asli')
    plt.plot(x_pred, y_pred, 'r--', label=f'Fit Polinomial derajat {degree}')
    # Tampilkan label x sebagai tanggal
